...
#new_model = tf.keras.models.load_model('saved_model_new_1/my_model11')
#np.random.seed(1)

# load deep learning model
json_file = open('model_loop1_21.json', 'r')
loaded_model_json = json_file.read()
json_file.close()
new_model = model_from_json(loaded_model_json)
# load weights into new model
new_model.load_weights("model_loop1_21.h5")
...
np.random.seed(10)
def Deep_data_assimilation(yo, DA):
    """ 
    ...
    if (DA.method =='AnEnKF'):
        ...
        Pf = np.zeros([T,n,n])
        xf1=np.zeros([T,DA.N,n])
        xf2=np.zeros([T,DA.N,n])
        
        #print(x_hat.part[:].shape)
        #test_X = X_test.reshape((X_test.shape[0], 1, X_test.shape[1]))
        for k in tqdm(range(0,T)):
            x_new_hat.part = x_hat.part.reshape((T,x_hat.part.shape[1], 1, x_hat.part.shape[2]))
            ...
            if k==0:
                ...
                #print(xf1.shape)
                #x_new_hat.part=x_hat.part
                
                xf=new_model.predict(x_new_hat.part[k-1,:,:,:])
                xf=xf.reshape((DA.N,n))
                #xf2[k-1,:,:]=xf
                #print("x_hat")
                #print(x_hat.part[k-1,:,:,:].shape)
                #print("xf shape")
                #print(xf.shape)
                #print("deep learning")
                #print(xf2)
            ...
            if (len(i_var_obs)>0):                
                ...
                #x_new_hat.part[k,:,:] = xf + np.dot(d,K.T)
    ...
    # particle filter method
        ...
        for k in tqdm(range(1,T)):
            ...
            xf2=new_model.predict(x_new_hat.part[k-1,:,:,:])
            xf=xf2.reshape((DA.N,n))
            xf=xf+np.random.multivariate_normal(np.zeros(xf.shape[1]),DA.B/100.0,xf.shape[0])
            
            #print(x_hat.part[k-1,:,:])
